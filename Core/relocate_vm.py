    ...
    try:
        ...
        if host_dest is not None:
            if current_host == host_dest:
                raise Exception(
                    "WARNING:: destination_host can not equal " "current_host"
                )
        ...
        message = "relocate_vm failed for vm:" + vm_name + " with error:" + str(e)
    print(message)
    return relocation_status, message


def main(si, datastore_name, esx_name, vm_name):
    try:
